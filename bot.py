...
def score_setup(indicators, side="LONG"):
    ...
    if rsi_v < 30:
        if side == "LONG": score += rsi_w; reasons.append("RSI oversold")
        else: score -= rsi_w; reasons.append("RSI oversold (bad for short)")
    elif rsi_v > 70:
        if side == "LONG": score += rsi_w; reasons.append("RSI overbought")
        else: score += rsi_w; reasons.append("RSI overbought (good for short)")

    ema_trend = indicators.get("ema_trend", "neutral")
    ema_w = 0.15 + adj.get("EMA", 0.0)
    if ema_trend == "up":
        if side == "LONG": score += ema_w; reasons.append("EMA up")
        else: score -= ema_w; reasons.append("EMA up (bad for short)")
    elif ema_trend == "down":
        if side == "LONG": score -= ema_w; reasons.append("EMA down")
        else: score += ema_w; reasons.append("EMA down (good for short)")

    bb_pos = indicators.get("bb_position", "middle")
    bb_w = 0.10 + adj.get("Bollinger", 0.0)
    if bb_pos == "above_upper":
        if side == "LONG": score += bb_w; reasons.append("BB above upper")
        else: score += bb_w; reasons.append("BB above upper (good for short)")
    elif bb_pos == "below_lower":
        if side == "LONG": score += bb_w; reasons.append("BB below lower")
        else: score -= bb_w; reasons.append("BB below lower (bad for short)")
    ...
    # Add Smart Money Concepts structure adjustment
    smc_data = indicators.get("smc", {}) or {}
    struct = smc_data.get("structure_strength", 0.0) or 0.0
    if struct != 0:
        if (side == "LONG" and struct > 0) or (side == "SHORT" and struct < 0):
            score = min(1.0, score + abs(struct) * 0.05)
            smc_desc = smc_data.get("description", "")
            if smc_desc and not any("SMC" in r for r in reasons):
                reasons.append(f"SMC {smc_desc}")
        elif (side == "LONG" and struct < 0) or (side == "SHORT" and struct > 0):
            score = max(0.0, score - abs(struct) * 0.03)
    return apply_ml_adjustment(score, indicators, side), reasons
...
def scan_market():
    ...
    for sym, ch24, ch1h, klines in fetched:
        ...
        try:
            smc_features = get_smc_features(klines)
            indicators["smc"] = smc_features
        except Exception as e:
            indicators["smc"] = {}
            print(f"  smc error for {sym}: {e}")
        # Tighter side selection: require BOTH signals for LONG, distinct setup for SHORT
        long_signal = (rsi_v < 50 and ema_trend == "up")
        short_signal = (rsi_v > 55 and ema_trend == "down" and mom_1h < -0.5)
        if long_signal:
            side = "LONG"
        elif short_signal:
            side = "SHORT"
        else:
            continue  # ambiguous setup, skip
...
def broadcast_signals(signals, to_admin=False):
    ...
    for sig in signals:
        if "close_price" in sig:
            ...
            smc_text = sig.get("indicators", {}).get("smc", {}).get("description", "")
            if smc_text:
                text += f"Structure: {smc_text}\n"
            text += "\n"
